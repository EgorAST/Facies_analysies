...
def pearson_correlation(sequence1: np.ndarray, sequence2: np.ndarray) -> float:
    """
    ...
    ----------
    float
        Коэффициент корреляции Пирсона между двумя последовательностями.
    """
    if len(sequence1) != len(sequence2):
        raise ValueError("Последовательности должны иметь одинаковую длину.")

    # Вычисляем коэффициент корреляции Пирсона
    correlation = np.corrcoef(sequence1, sequence2, rowvar=False)[0, 1]
...
def find_best_match(fragment: np.ndarray, full_curve: np.ndarray)-> Union[int, float]:
    ...
    for i in range(len(full_curve) - fragment_length + 1):
        ...
        distance = pearson_correlation(current_segment_norm, fragment_norm)
