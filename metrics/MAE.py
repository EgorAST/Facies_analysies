...
def calculate_mae(sequence1: np.ndarray, sequence2: np.ndarray) -> float:
    """
    Вычисляет среднюю абсолютную ошибку (MAE) между двумя последовательностями.
    ...
    ----------
    float
        Средняя абсолютная ошибка между двумя последовательностями.
    """
    if len(sequence1) != len(sequence2):
        raise ValueError("Последовательности должны иметь одинаковую длину.")

    # Вычисляем абсолютные разности между соответствующими точками
    absolute_diff = np.abs(sequence1 - sequence2)

    # Возвращаем среднее значение абсолютных разностей
...
def find_best_match(fragment: np.ndarray, full_curve: np.ndarray)-> Union[int, float]:
    ...
    for i in range(len(full_curve) - fragment_length + 1):
        ...
        distance = calculate_mae(current_segment_norm, fragment_norm)
